...
namespace L10_FudgeCraft_DetectCombos {
    ...
    export class Grid extends Map<string, GridElement> {
        ...
        public findNeighbors(_of: f.Vector3): GridElement[] {
            return [-1, 1]
                .flatMap((_element: number) => {
                    return [
                        grid.pull(f.Vector3.SUM(_of, f.Vector3.X(_element))),
                        grid.pull(f.Vector3.SUM(_of, f.Vector3.Y(_element))),
                        grid.pull(f.Vector3.SUM(_of, f.Vector3.Z(_element)))
                    ];
                })
                .filter((_element: GridElement) => {
                    return _element != null;
